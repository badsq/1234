...
class RLAgent:
    ...
    def act(self, obs: np.ndarray) -> Tuple[np.ndarray, torch.Tensor, torch.Tensor, torch.Tensor]:
        """Select actions for one or many observations.

        Supports both a single observation ``(obs_dim,)`` and a batch of
        observations ``(batch, obs_dim)``. In the batched case the returned
        action array has shape ``(batch,)`` and log-probabilities, entropies and
        values are tensors of the same shape.
        """
        ...
        if single:
            return actions.cpu().numpy()[0], logp[0], ent[0], value[0]
        return actions.cpu().numpy(), logp, ent, value
    ...
    def update(
        ...
        G = torch.zeros_like(rewards[0], dtype=torch.float32, device=self.device)
        for r in reversed(rewards):
            G = r.to(self.device) + gamma * G
            returns.insert(0, G)
