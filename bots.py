...
class PolicyNet(nn.Module):
    def __init__(self, obs_dim: int, hidden: int = 1024, n_actions: int = 10):
        super().__init__()
...
class RLAgent:
    ...
    def __init__(
        ...
        self.policy = PolicyNet(obs_dim, hidden=hidden).to(self.device)
        self.value = ValueNet(obs_dim, hidden=hidden).to(self.device)
        self.params = list(self.policy.parameters()) + list(self.value.parameters())
