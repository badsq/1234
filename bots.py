...
class RLAgent:
    ...
    def update(
        ...
        """Update policy and value networks from episode logs using GAE."""
        rewards_t = torch.tensor(rewards, dtype=torch.float32, device=self.device)
        values_t = torch.stack(values).to(self.device)
        values_ext = torch.cat([values_t, torch.zeros(1, device=self.device)])
        adv = torch.zeros_like(values_t)
        ret = torch.zeros_like(values_t)
        gae = 0.0
        next_value = 0.0
        for t in reversed(range(len(rewards))):
            next_value = values_ext[t + 1]
            delta = rewards_t[t] + gamma * next_value - values_ext[t]
        ...
        policy_adv = (adv.detach() - adv.mean()) / (adv.std() + 1e-8)
        policy_loss = -(log_probs_t * policy_adv).mean()
        value_loss = (ret - values_t).pow(2).mean()
    ...
    def update_batch(
        ...
        values_ext = torch.cat(
            [values, torch.zeros(batch, 1, device=self.device)], dim=1
        )
        adv = torch.zeros_like(rewards)
        ret = torch.zeros_like(rewards)
        gae = torch.zeros(batch, device=self.device)
        ...
        returns_t = ret.reshape(-1)
        adv_t = adv.reshape(-1)
        entropies_t = entropies.reshape(-1)
        policy_adv = (adv_t.detach() - adv_t.mean()) / (adv_t.std() + 1e-8)
        policy_loss = -(log_probs_t * policy_adv).mean()
